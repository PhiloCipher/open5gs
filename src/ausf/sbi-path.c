...
int ausf_sbi_open(void)
{
    ...
    if (ogs_sbi_nf_service_is_available(OGS_SBI_SERVICE_NAME_NAUSF_REPORT)) {
        service = ogs_sbi_nf_service_build_default(
                    nf_instance, OGS_SBI_SERVICE_NAME_NAUSF_REPORT);
        ogs_assert(service);
        ogs_sbi_nf_service_add_version(
                    service, OGS_SBI_API_V1, OGS_SBI_API_V1_0_0, NULL);
        ogs_sbi_nf_service_add_allowed_nf_type(service, OpenAPI_nf_type_TDF);// If the num_of_allowed_nf_type of a service is zero, every NF can access.
    }
