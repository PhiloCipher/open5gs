...
void lmf_state_operational(ogs_fsm_t *s, lmf_event_t *e)
{
    ...
    ogs_assert(s);
    ogs_ad("LMF state %d: %s", e->h.id, lmf_event_get_name(e));
    ...
    case OGS_EVENT_SBI_SERVER:
        ...
        CASE(OGS_SBI_SERVICE_NAME_NLMF_LOC)
            ogs_ad("TDF OGS_SBI_SERVICE_NAME_NLMF_LOC: %s", message.h.resource.component[0]);
            SWITCH(message.h.resource.component[0])
            CASE(OGS_SBI_RESOURCE_NAME_DETERMINE_LOCATION)
                lmf_nlmf_loc_handle_determine_location(stream, &message);
                break;
