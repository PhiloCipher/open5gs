...
int tgf_sbi_open(void)
{
    ...
    // if (ogs_sbi_nf_service_is_available(OGS_SBI_SERVICE_NAME_NTGF_UEAU)) {
    //     service = ogs_sbi_nf_service_build_default(
    //                 nf_instance, OGS_SBI_SERVICE_NAME_NTGF_UEAU);
    //     ogs_assert(service);
    ...
    // if (ogs_sbi_nf_service_is_available(OGS_SBI_SERVICE_NAME_NTGF_UECM)) {
    //     service = ogs_sbi_nf_service_build_default(
    //                 nf_instance, OGS_SBI_SERVICE_NAME_NTGF_UECM);
    //     ogs_assert(service);
    //     ogs_sbi_nf_service_add_version(
    //                 service, OGS_SBI_API_V1, OGS_SBI_API_V1_0_0, NULL);
    ...
    // if (ogs_sbi_nf_service_is_available(OGS_SBI_SERVICE_NAME_NTGF_SDM)) {
    //     service = ogs_sbi_nf_service_build_default(
    //                 nf_instance, OGS_SBI_SERVICE_NAME_NTGF_SDM);
    //     ogs_assert(service);
    //     ogs_sbi_nf_service_add_version(
...
static int tgf_sbi_discover_and_send(
        ...
        void *context, ogs_sbi_stream_t *stream, void *data)
{
    ...
    if (!xact) {
        ogs_error("tgf_sbi_discover_and_send() failed");
        return OGS_ERROR;
    }
        ...
        tgf_ue_t *tgf_ue, ogs_sbi_stream_t *stream, void *data)
{
    ...
    r = tgf_sbi_discover_and_send(
                &tgf_ue->sbi, service_type, discovery_option,
                (ogs_sbi_build_f)build, tgf_ue, stream, data);
    if (r != OGS_OK) {
        ogs_error("tgf_ue_sbi_discover_and_send() failed");
        ogs_assert(true ==
            ogs_sbi_server_send_error(stream,
                OGS_SBI_HTTP_STATUS_GATEWAY_TIMEOUT, NULL,
                "Cannot discover", tgf_ue->suci, NULL));
        return r;
    }
        ...
        tgf_sess_t *sess, ogs_sbi_stream_t *stream, void *data)
{
    ...
    if (r != OGS_OK) {
        ogs_error("tgf_sess_sbi_discover_and_send() failed");
        ogs_assert(true ==
            ogs_sbi_server_send_error(stream,
                OGS_SBI_HTTP_STATUS_GATEWAY_TIMEOUT, NULL,
