...
const char *level_strings[] = {
    NULL,
    "FATAL", "ERROR", "Temp", "Communications", "Anonymous 5G","WARNING", "INFO", "DEBUG", "TRACE",
};
...
static ogs_log_level_e ogs_log_level_from_string(const char *string)
{
    ...
    else if (!strcasecmp(string, "tmp")) level = OGS_LOG_TMP;
    else if (!strcasecmp(string, "com")) level = OGS_LOG_COM;
    else if (!strcasecmp(string, "ad")) level = OGS_LOG_AD;
