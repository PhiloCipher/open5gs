...
cJSON *OpenAPI_event_notify_data_convertToJSON(OpenAPI_event_notify_data_t *event_notify_data)
{
    ...
    if (event_notify_data->reported_event_type == OpenAPI_reported_event_type_NULL) {
        ogs_error("OpenAPI_event_notify_data_convertToJSON() failed [reported_event_type]");
        return NULL;
    }
    if (cJSON_AddStringToObject(item, "reportedEventType", OpenAPI_reported_event_type_ToString(event_notify_data->reported_event_type)) == NULL) {
        ogs_error("OpenAPI_event_notify_data_convertToJSON() failed [reported_event_type]");
        goto end;
    }
    ...
    if (event_notify_data->termination_cause != OpenAPI_termination_cause_NULL) {
    if (cJSON_AddStringToObject(item, "terminationCause", OpenAPI_termination_cause_ToString(event_notify_data->termination_cause)) == NULL) {
...
OpenAPI_event_notify_data_t *OpenAPI_event_notify_data_parseFromJSON(cJSON *event_notify_dataJSON)
{
    ...
    reported_event_typeVariable = OpenAPI_reported_event_type_FromString(reported_event_type->valuestring);
    ...
    if (!cJSON_IsString(termination_cause)) {
        ogs_error("OpenAPI_event_notify_data_parseFromJSON() failed [termination_cause]");
        goto end;
    }
    termination_causeVariable = OpenAPI_termination_cause_FromString(termination_cause->valuestring);
    }
