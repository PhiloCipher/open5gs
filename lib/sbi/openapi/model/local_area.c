...
cJSON *OpenAPI_local_area_convertToJSON(OpenAPI_local_area_t *local_area)
{
    ...
    if (local_area->shape == OpenAPI_supported_gad_shapes_NULL) {
        ogs_error("OpenAPI_local_area_convertToJSON() failed [shape]");
        return NULL;
    }
    if (cJSON_AddStringToObject(item, "shape", OpenAPI_supported_gad_shapes_ToString(local_area->shape)) == NULL) {
        ogs_error("OpenAPI_local_area_convertToJSON() failed [shape]");
        goto end;
    }
