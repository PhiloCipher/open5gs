...
{
    const char *reporting_access_typeArray[] =  { "NULL", "NR", "EUTRA_CONNECTED_TO_5GC", "NON_3GPP_CONNECTED_TO_5GC", "NR_LEO", "NR_MEO", "NR_GEO", "NR_OTHER_SAT" };
    size_t sizeofArray = sizeof(reporting_access_typeArray) / sizeof(reporting_access_typeArray[0]);
...
OpenAPI_reporting_access_type_e OpenAPI_reporting_access_type_FromString(char* reporting_access_type)
{
    ...
    while (stringToReturn < sizeofArray) {
        if (strcmp(reporting_access_type, reporting_access_typeArray[stringToReturn]) == 0) {
