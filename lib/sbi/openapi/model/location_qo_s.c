...
cJSON *OpenAPI_location_qo_s_convertToJSON(OpenAPI_location_qo_s_t *location_qo_s)
{
    ...
    if (location_qo_s->response_time != OpenAPI_response_time_NULL) {
    if (cJSON_AddStringToObject(item, "responseTime", OpenAPI_response_time_ToString(location_qo_s->response_time)) == NULL) {
    ...
    if (location_qo_s->lcs_qos_class != OpenAPI_lcs_qos_class_NULL) {
    if (cJSON_AddStringToObject(item, "lcsQosClass", OpenAPI_lcs_qos_class_ToString(location_qo_s->lcs_qos_class)) == NULL) {
...
OpenAPI_location_qo_s_t *OpenAPI_location_qo_s_parseFromJSON(cJSON *location_qo_sJSON)
{
    ...
    if (!cJSON_IsString(response_time)) {
        ogs_error("OpenAPI_location_qo_s_parseFromJSON() failed [response_time]");
        goto end;
    }
    ...
    if (!cJSON_IsString(lcs_qos_class)) {
        ogs_error("OpenAPI_location_qo_s_parseFromJSON() failed [lcs_qos_class]");
        goto end;
    }
