...
cJSON *OpenAPI_motion_event_info_convertToJSON(OpenAPI_motion_event_info_t *motion_event_info)
{
    ...
    if (motion_event_info->occurrence_info != OpenAPI_occurrence_info_NULL) {
    if (cJSON_AddStringToObject(item, "occurrenceInfo", OpenAPI_occurrence_info_ToString(motion_event_info->occurrence_info)) == NULL) {
...
OpenAPI_motion_event_info_t *OpenAPI_motion_event_info_parseFromJSON(cJSON *motion_event_infoJSON)
{
    ...
    OpenAPI_occurrence_info_e occurrence_infoVariable = 0;
    ...
    if (!cJSON_IsString(occurrence_info)) {
        ogs_error("OpenAPI_motion_event_info_parseFromJSON() failed [occurrence_info]");
        goto end;
    }
    occurrence_infoVariable = OpenAPI_occurrence_info_FromString(occurrence_info->valuestring);
    }
