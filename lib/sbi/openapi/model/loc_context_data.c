...
cJSON *OpenAPI_loc_context_data_convertToJSON(OpenAPI_loc_context_data_t *loc_context_data)
{
    ...
    if (loc_context_data->supported_gad_shapes != OpenAPI_supported_gad_shapes_NULL) {
    ...
    if (loc_context_data->ldr_type == OpenAPI_ldr_type_NULL) {
        ogs_error("OpenAPI_loc_context_data_convertToJSON() failed [ldr_type]");
        return NULL;
    }
    if (cJSON_AddStringToObject(item, "ldrType", OpenAPI_ldr_type_ToString(loc_context_data->ldr_type)) == NULL) {
        ogs_error("OpenAPI_loc_context_data_convertToJSON() failed [ldr_type]");
        goto end;
    }
...
OpenAPI_loc_context_data_t *OpenAPI_loc_context_data_parseFromJSON(cJSON *loc_context_dataJSON)
{
    ...
    if (supported_gad_shapes) {
        ...
        if (supported_gad_shapesList->count == 0) {
            ogs_error("OpenAPI_loc_context_data_parseFromJSON() failed: Expected supported_gad_shapesList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }
