...
{
    const char *lcs_priorityArray[] =  { "NULL", "HIGHEST_PRIORITY", "NORMAL_PRIORITY" };
    size_t sizeofArray = sizeof(lcs_priorityArray) / sizeof(lcs_priorityArray[0]);
...
OpenAPI_lcs_priority_e OpenAPI_lcs_priority_FromString(char* lcs_priority)
{
    ...
    while (stringToReturn < sizeofArray) {
        if (strcmp(lcs_priority, lcs_priorityArray[stringToReturn]) == 0) {
