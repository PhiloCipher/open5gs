...
{
    const char *positioning_modeArray[] =  { "NULL", "UE_BASED", "UE_ASSISTED", "CONVENTIONAL" };
    size_t sizeofArray = sizeof(positioning_modeArray) / sizeof(positioning_modeArray[0]);
...
OpenAPI_positioning_mode_e OpenAPI_positioning_mode_FromString(char* positioning_mode)
{
    ...
    while (stringToReturn < sizeofArray) {
        if (strcmp(positioning_mode, positioning_modeArray[stringToReturn]) == 0) {
