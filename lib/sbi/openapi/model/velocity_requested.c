...
{
    const char *velocity_requestedArray[] =  { "NULL", "VELOCITY_IS_NOT_REQUESTED", "VELOCITY_IS_REQUESTED" };
    size_t sizeofArray = sizeof(velocity_requestedArray) / sizeof(velocity_requestedArray[0]);
...
OpenAPI_velocity_requested_e OpenAPI_velocity_requested_FromString(char* velocity_requested)
{
    ...
    while (stringToReturn < sizeofArray) {
        if (strcmp(velocity_requested, velocity_requestedArray[stringToReturn]) == 0) {
