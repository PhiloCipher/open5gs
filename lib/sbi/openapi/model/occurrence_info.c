...
{
    const char *occurrence_infoArray[] =  { "NULL", "ONE_TIME_EVENT", "MULTIPLE_TIME_EVENT" };
    size_t sizeofArray = sizeof(occurrence_infoArray) / sizeof(occurrence_infoArray[0]);
...
OpenAPI_occurrence_info_e OpenAPI_occurrence_info_FromString(char* occurrence_info)
{
    ...
    while (stringToReturn < sizeofArray) {
        if (strcmp(occurrence_info, occurrence_infoArray[stringToReturn]) == 0) {
