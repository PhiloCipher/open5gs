...
DetermineLocationAPI_determineLocation(apiClient_t *apiClient, input_data_t * input_data )
{
    ...
    // create the path
    long sizeOfPath = strlen("/determine-location")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/determine-location");
    ...
    {
        //string
        localVarSingleItemJSON_input_data = input_data_convertToJSON(input_data);
        localVarBodyParameters = cJSON_Print(localVarSingleItemJSON_input_data);
    }
    ...
    //    printf("%s\n","Expected response for MO-LR requesting location assistance data.");
    //}
    // uncomment below to debug the error response
    ...
    cJSON *DetermineLocationAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    location_data_t *elementToReturn = location_data_parseFromJSON(DetermineLocationAPIlocalVarJSON);
    cJSON_Delete(DetermineLocationAPIlocalVarJSON);
